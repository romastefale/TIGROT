...
# Correção Log: Enviando log para stdout e ocultando poluição do httpx
logging.basicConfig(
...
# =========================
# FUNÇÕES DE HIGIENIZAÇÃO (Corrigido para Async)
# =========================
async def sanitize_text(text):
    ...
    Feito de forma assíncrona para não travar o bot.
    """
    if not text:
        return text
...
async def handle_log_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "log_edit":
        context.user_data["awaiting_log"] = True
        await cb_query.answer("Modo de edição ativado.")
        try:
            await cb_query.edit_message_reply_markup(reply_markup=None)
        except Exception:
            pass
...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_name = escape_markdown(await sanitize_text(user.first_name if user else "Someone"))

    results = []
    for i, track in enumerate(tracks[:10]):
        try:
            clean_title = await sanitize_text(track["title"])
            clean_artist = await sanitize_text(track["artist"]["name"])
            clean_album = await sanitize_text(track["album"]["title"])
            
            title = escape_markdown(clean_title)
            artist = escape_markdown(clean_artist)
            album = escape_markdown(clean_album)
            ...
            results.append(
                InlineQueryResultPhoto(
                    ...
                    photo_url=cover,
                    thumbnail_url=cover,
                    title=f"{clean_title} — {clean_artist}",
...
async def send_results(update, context):
    ...
    for i, track in enumerate(tracks[:10]):
        title = await sanitize_text(track["title"])
        artist = await sanitize_text(track["artist"]["name"])
...
# =========================
# ESCOLHER MÚSICA (E Validação de Letra)
# =========================
